...
動的[[ルーティング]]プロトコルは、ネットワークの経路情報を自動的に交換・学習する仕組みです。この記事では、企業ネットワークで頻出の[[OSPF]]と[[BGP]]を中心に、設計ポイントを整理します。
...
[[OSPF]]はリンクステート型の[[IGP]]で、企業ネットワークで最も広く使われています。
...
[[BGP]]はAS（自律システム）間の経路交換に使用される[[EGP]]です。
